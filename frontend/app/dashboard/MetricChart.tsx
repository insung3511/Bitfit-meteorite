...
function ChartTooltip({
  ...
  const value = typeof raw === "number" ? formatValue(raw) : String(raw ?? "");
  return (
    <div className="rounded-md border border-black/10 bg-white px-2.5 py-1.5 text-xs shadow-sm dark:border-white/15 dark:bg-neutral-900">
      ...
      <div className="text-black/60 dark:text-white/60">
        7-day mean: <span className="font-mono">{value}</span>
      </div>
    </div>
  );
}
 ...
 */
export default function MetricChart({ label, color, points }: Props) {
  const gradientId = `grad-${label.replace(/[^a-z0-9]/gi, "")}`;
  const hasData = points.some((p) => p.mean_7d != null);
